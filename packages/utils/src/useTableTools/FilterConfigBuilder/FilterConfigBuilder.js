...
class FilterConfigBuilder {
  ...
  addConfigItem = (item) => (this.config = this.config.filter((i) => i.label !== item.label).concat(item));
  ...
  buildConfiguration = (handler, states, props = {}, initConfig) => ({
    ...props,
    items: (initConfig || this.config).map((item) => this.toFilterConfigItem(item, handler, states[stringToId(item.label)])).filter((v) => !!v),
  });
  ...
  initialDefaultState = (defaultStates = {}, initConfig) =>
    (initConfig || this.config).reduce((acc, filter) => {
      ...
      const state = defaultStates[filterStateName] || this.defaultValueForFilter(filter);
  ...
  categoryLabelForValue = (value) => {
    const category = this.config.filter((category) => (category.items ? category.items.map((item) => item.value).includes(value) : false))[0];

    return category ? category.label : value;
  };

  getCategoryForLabel = (query) => this.config.filter((item) => stringToId(item.key || item.label) === stringToId(query))[0] || {};
  ...
  getItemByLabelOrValue = (query, category) => {
    ...
    const results = (items || []).filter((item) => item.value === query || item.label === query);
  ...
  removeFilterFromFilterState = (currentState, filter) => (typeof currentState === 'string' ? '' : currentState.filter((value) => value !== filter));
  ...
  removeFilterWithChip = (chips, activeFilters) => {
    ...
    const newFilterState = (isGroup ? this.removeFilterFromGroupFilterState : this.removeFilterFromFilterState)(currentState, chipValue, chipItem);
