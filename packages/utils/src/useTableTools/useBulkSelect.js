...
const mergeArraysUniqly = (arrayA, arrayB) => Array.from(new Set([...arrayA, ...arrayB]));

export const useBulkSelect = ({ total, onSelect, preselected, itemIdsInTable, itemIdsOnPage, identifier = 'id' }) => {
  const enableBulkSelect = !!onSelect;
  // TODO This useState and all that works with it could be moved into a separate hook; just for readability.
  ...
  const selectOne = (_, selected, key, row) => onSelectCallback(() => (selected ? selectItems([row[identifier]]) : unselectItems([row[identifier]])));
...
export const useBulkSelectWithItems = ({ onSelect, items: propItems, filter, paginator, preselected, setPage }) => {
  const enableBulkSelect = !!onSelect;
