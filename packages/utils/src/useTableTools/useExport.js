import { camelCase, getProperty } from './helper';
...
const filename = (format) => CSV_FILE_PREFIX + '-' + new Date().toISOString() + '.' + format;
...
const useExport = ({ exporter, columns = [], isDisabled = false, onStart, onComplete, onError }) => {
  const exportableColumns = columns.filter((column) => column.export !== false && (column.exportKey || column.renderExport));
...
export const useExportWithItems = (items, columns, options = {}) => {
  ...
  const { columns: exportableColumns, onStart, onComplete } = typeof options.exportable === 'object' ? options.exportable : {};
  const exportableSelectedColumns = (exportableColumns || columns).filter((column) => columns.includes(column));
