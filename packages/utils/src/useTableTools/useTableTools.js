...
const useTableTools = (items = [], columns = [], options = {}) => {
  ...
  const { toolbarProps: pagintionToolbarProps, setPage, paginator } = usePaginate(options);
  ...
  const { tableProps: sortableTableProps, sorter } = useTableSortWithItems(items, managedColumns, options);
  ...
  const { toolbarProps: rowBuilderToolbarProps, tableProps: rowBuilderTableProps } = useRowsBuilder(identifiedItems, sortableTableProps.cells, {
